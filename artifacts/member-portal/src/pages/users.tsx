...
export default function Users() {
  ...
  const createUser = useCreateUser({
    mutation: {
      onSuccess: () => { invalidate(); setDialogOpen(false); toast({ title: "User created" }); },
      onError: (e: unknown) => {
  ...
  const updateUser = useUpdateUser({
    mutation: {
      onSuccess: () => { invalidate(); setDialogOpen(false); toast({ title: "User updated" }); },
  ...
  const deleteUser = useDeleteUser({
    mutation: {
      onSuccess: () => { invalidate(); setDeleteId(null); toast({ title: "User deleted" }); },
    },
  });
  ...
  return (
    <div className="p-8 space-y-6">
              ...
              <TableBody>
                {users.map((u) => (
                  <TableRow key={u.id}>
                    ...
                    <TableCell>
                      <Badge className={u.role === "admin" ? "bg-blue-100 text-blue-700 border-blue-200" : "bg-gray-100 text-gray-600 border-gray-200"}>
                        {u.role}
                      </Badge>
                    </TableCell>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editId != null ? "Edit User" : "Add User"}</DialogTitle>
          </DialogHeader>
      ...
      <AlertDialog open={deleteId != null} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction className="bg-destructive text-destructive-foreground hover:bg-destructive/90" onClick={() => deleteId && deleteUser.mutate({ id: deleteId })}>
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
