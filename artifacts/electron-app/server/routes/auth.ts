...
router.post("/login", async (req, res) => {
  ...
  if (hashPassword(String(password)) !== user.passwordHash) {
...
router.post("/change-password", requireAuth, async (req, res) => {
  ...
  await db.update(usersTable).set({ passwordHash: hashPassword(String(newPassword)) }).where(eq(usersTable.id, req.user!.userId));
