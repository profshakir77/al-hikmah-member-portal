...
import { contributionsTable } from "../schema.js";
...
router.get("/", async (req, res) => {
  ...
  const rows = whereClause
    ? await db.select().from(contributionsTable).where(whereClause).orderBy(desc(contributionsTable.date))
...
router.post("/", async (req, res) => {
  ...
  const [row] = await db.insert(contributionsTable).values({
    name, phone: phone || null, amount: String(amount), date,
    type: type ?? "participant", reference: reference || null, notes: notes || null,
    year: Number(year), month: month ? Number(month) : null,
  }).returning();
